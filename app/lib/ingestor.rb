class Ingestor
  class << self
      ...
      (1..1000).each do |page|
        tournaments, cursor = if before_date.present?
          ...
          provider.tournaments(
            ...
            cursor:,
            before_date:,
            sort_order: Provider::SORT_ORDER_NEWEST_FIRST
          )
        else
          ...
          provider.tournaments(
            ...
            sort_order: Provider::SORT_ORDER_OLDEST_FIRST
          )
        end
    ...
    def provider
      Provider.provider(self::PROVIDER_NAME)
    end

    def factory
      Factory.factory(self::PROVIDER_NAME)
    end
