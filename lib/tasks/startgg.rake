namespace :startgg do
  ...
  task sync_tournaments: [:environment] do
    Startgg::Ingestor.sync_tournaments
  end

  task sync_overrides: [:environment] do
    Startgg::Ingestor.sync_overrides
  end

  task sync_entrants: [:environment] do
    Startgg::Ingestor.sync_entrants
  end

  task sync_sets: [:environment] do
    Startgg::Ingestor.sync_sets
  end
  ...
  task sync_past_tournaments: [:environment] do
    ...
    next if Tournament.where(provider: Startgg::Provider::PROVIDER_NAME).should_display.in_progress.any?

    cursor_date = Rails.cache.read('startgg/past_tournaments_cursor_date') || Time.now

    Startgg::Ingestor.sync_tournaments(before_date: cursor_date + 30.minutes, limit: 100, sync_entrants: true, delete_if_shouldnt_display: true) do |tournament|
